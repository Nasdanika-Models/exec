package org.nasdanika.models.exec.generator.tests;
...
import org.nasdanika.models.exec.processors.EcoreGenExecProcessorsFactory;
 ...
 *
 */
public class TestExecModelDocGen {
	
	@Test
	public void testGenerateExecModelDoc() throws IOException, DiagnosticException {
		List<EPackage> ePackages = Arrays.asList(EcorePackage.eINSTANCE, NcorePackage.eINSTANCE, ExecPackage.eINSTANCE);
		...
		EcoreGenExecProcessorsFactory ecoreGenExecProcessorFactory = new EcoreGenExecProcessorsFactory(context);		
		...
		Map<EPackage, URI> packageURIMap = Map.ofEntries(
			...
			Map.entry(ExecPackage.eINSTANCE, baseActionURI)	
		);
				...
				if (element instanceof EObjectNode) {
					...
					if (target == topLevelPackage) {
						...
						if (processor instanceof WidgetFactory) {
							...
							if (topLevelPackage == ExecPackage.eINSTANCE) { 							
		...
		File output = new File(actionModelsDir, "exec.xmi");
